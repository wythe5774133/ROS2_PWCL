...
ipaddr = '192.168.10.3' #LHU computer
port = 80
...
class DroneTimerTaskNode(Node):
    def __init__(self):
        ...
        self.sendAce6RSSI_task = self.create_timer(1/1,self.send_Ace6_RSSI_value)
        # self.seetarget_task = self.create_timer(1/30,self.see_target)
        self.service_client = DroneServiceNode()
        
    def send_Ace6_RSSI_value(self):
        global drone_id
        head_code = b'\x52\x53\x53\x49'
        rssi = RSSI_Value.get_Ace6_rssi()
        if rssi == None:
            rssi = 404
        end_code = b'\x06'
